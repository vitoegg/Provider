...
cd ~/
wget --no-check-certificate -O snell.zip https://github.com/surge-networks/snell/releases/download/v2.1.0b1/snell-server-v2.1.0-linux-aarch64.zip
unzip -o snell.zip
rm -f snell.zip
...
if [ -f ${CONF} ]; then
  ...
  echo "listen = 0.0.0.0:59188" >>${CONF}
  echo "psk = ${PSK}" >>${CONF}
  echo "obfs = http" >>${CONF}
fi
if [ -f ${SYSTEMD} ]; then
